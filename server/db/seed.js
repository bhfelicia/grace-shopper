...
const seed = async () => {
  try {
    ...
    const flowerPop = await Product.create({
      name: "Flower Pop",
      description:
        "This bright vase colorfully graces any table with style and is beautiful, with and without flowers!",
      price: 100,
      size: "medium",
      categoryId: vases.id,
    });
    ...
    const kickItUp = await Product.create({
      ...
      description:
        "Though it may be one color, there's nothing monotone about this teapot, which packs a punch of pizzaz. Wow your guests with this whimsical piece reminiscient of Alice in Wonderland.",
      price: 65,
      size: "medium",
      categoryId: teapots.id,
      image:
    ...
    const waveItOut = await Product.create({
      name: "Wave It Out",
      description:
        "The sea comes home with you when you in this elegant replica of a wave that connotes motion and carefree summer beach days",
      price: 200,
      size: "medium",
      categoryId: sculptures.id,
      image:
